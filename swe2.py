...
def shallow_water_1d_test(nx_init=20,nt_init=500,distance=1,g_init=9.8):
    # *****************************************************************************80
    #
...
def shallow_water_1d(nx, nt, x_length, t_length, g):
    ...
    #  Parameters:
    #
    #    Input, integer NX, the number of spatial nodes. (tmno del paso)
    #
    #    Input, integer NT, the number of times steps. (dt)
    #
    #    Input, real X_LENGTH, the length of the region. (vertices a lo largo )
    #
    #    Input, real T_LENGTH, the time extent. (tiempo animacion)
    #
    ...
    for it in range(1, nt + 1):
        ...
        uh[1:nx - 1] = uh[1:nx - 1] \
                       - dt * (uhm[1:nx - 1] ** 2 / hm[1:nx - 1] + 0.5 * g * hm[1:nx - 1] ** 2 -
...
def initial_conditions(nx, nt, h, uh, x):
    ...
    h = 2.0 + np.cos(2.0 * np.pi * x)

    #h =(1*np.random.uniform(0,1))+2.0 + np.sin(2.0 * np.pi * x)
